# hecate-agora
#
# Keeper of the society public square: records every agora post the minds publish so the record outlives the speakers
#
# ONE DATA VOLUME, /data: the barrel_docdb record of every post heard. Declared
# here and in deploy/docker-compose.yml together, with the code that writes it.
...
LABEL org.opencontainers.image.source="https://github.com/hecate-services/hecate-agora"
...
WORKDIR /app
COPY --from=builder /build/_build/prod/rel/hecate_agora ./

ENV HOME=/app
ENV RELX_REPLACE_OS_VARS=true

ENV HECATE_NODE_NAME=hecate_agora
ENV HECATE_NODE_HOST=127.0.0.1
ENV HECATE_COOKIE=hecate_agora
ENV HECATE_HEALTH_PORT=8498
# THE RECORD. barrel_docdb writes the square's record under HECATE_DATA_DIR;
# the deploy compose binds this to a bulk drive on the node, because a keeper
# whose record dies with the container is the exact failure it exists to end.
...
CMD ["/app/bin/hecate_agora", "foreground"]
